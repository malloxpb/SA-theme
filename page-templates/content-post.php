...
<article id="post-<?php the_ID(); ?>" <?php post_class("demo-card-wide mdl-card mdl-shadow--2dp"); ?>>
	<div class="mdl-card__title" style="background:url('<?php if (has_post_thumbnail() && (get_theme_mod('index_feat_image') != 1)) { the_post_thumbnail_url('sydney-post-thumb'); } ?>');background-size:auto;background-repeat:no-repeat;background-position: center;">
